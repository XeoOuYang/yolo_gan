...
class YoloV8Heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        method = eval(method)(model, target_layers, use_cuda=self.device.type == 'cuda')
        method.activations_and_grads = ActivationsAndGradients(model, target_layers, None)
        self.method = method

        self.colors = np.random.uniform(0, 255, size=(len(model_names), 3)).astype(np.short)
...
def get_params():
    params = {
        'weight': 'out/detect/best.pt',  # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
